...
func (t *TSPSolver) adaptiveSearch() ([]int, float64) {
	...
	for iteration := 0; iteration < 200; iteration++ {
		...
		t.bestDistance = math.MaxFloat64 // ВАЖНО: сбрасываем на максимум!
		t.visitedCount = 0
		...
		startTime := time.Now()
		t.bruteForceRecursive([]int{0}, 0.0, visited, currentThreshold) // Передаём порог!
...
func (t *TSPSolver) bruteForceRecursive(currentPath []int, currentDistance float64, visited []bool, threshold float64) {
	...
	for _, nextPoint := range pointsToTry {
		...
		t.bruteForceRecursive(newPath, newDistance, visited, threshold)
